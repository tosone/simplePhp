...
<head runat="server">
  <title>Tosone's Code Snippets - 搜索Keywords: <?=$_GET["keywords"]?></title>
...
<body onload="prettyPrint()">
  <div class="container">
    <?
      ...
      $tpage = ceil(intval($sqlite3db->querySingle('select count(id) from code where intro like "%'. $_GET["keywords"] .'%"')) / PAGE_NUM);
      $page = intval(isset($_GET["page"]) ? $_GET["page"] : 1);
      $page = ($page > $tpage || $page < 1) ? 1 : $page;
      if(!isset($_GET["keywords"]) || preg_match("/\b(and|or|exec|execute|insert|select|delete|update|alter|create|drop|count|\*|chr|char|asc|mid|substring|master|truncate|declare|xp_cmdshell|restore|backup|net +user|net +localgroup +administrators)\b/", $_GET["keywords"])) header("index.php");
    ...
    <?=($page == 1) ? '<a href="javascript:void(0);" class="f_btn f_ban">上一页</a>' : '<a href="index.php?searchKeyWords=' . $_GET["keywords"] . '&page=' . ($page - 1) .'" class="f_btn">上一页</a>' ?>
    <?='<a href="javascript:void(0);" class="f_btn f_num">' . $page . '&nbsp;/&nbsp;' . $tpage . '</a>' ?>
    <?=($page == $tpage) ? '<a href="javascript:void(0);" class="f_btn f_ban">下一页</a>' : '<a href="index.php?searchKeyWords=' . $_GET["keywords"] . '&page=' . ($page + 1) . '" class="f_btn">下一页</a>' ?>
    <?=($page == $tpage) ? '<a href="javascript:void(0);" class="f_btn f_ban">尾页</a>' : '<a href="index.php?searchKeyWords=' . $_GET["keywords"] . '&page='. $tpage . '" class="f_btn">尾页</a>' ?>
    </div>
  </div>
